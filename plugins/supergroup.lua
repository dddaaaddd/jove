...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'حله این گروه اضافه شد😊!'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'حله ریمو شد از لیست گروه ها😊'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local user_num = "😐تعداد کاربرا: "..result.participants_count.."\n"
...
local function lock_group_all(msg, data, target)
  ...
  if group_all_lock == 'yes' then
    return '*تمامی تنظیمات پیش از این قفل است'
  else
...
local function unlock_group_all(msg, data, target)
  ...
  if group_all_lock == 'no' then
    return '*تمامی تنظیمات پیش ازاین قفل نیست'
  else
    data[tostring(target)]['settings']['all'] = 'no'
    save_data(_config.moderation.data, data)
    return '*تمامی تنظیمات باز شد'
  end
end
...
local function lock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == 'yes' then
    return '*تنظیمات اتحاد پیش از این قفل است'
  else
...
local function unlock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == 'no' then
    return '*تنظیمات اتحاد پیش از این قفل نیست'
  else
...
local function lock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'yes' then
    return '*خارج شدن پیش از این قفل است'
  else
...
local function unlock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'no' then
    return '*خارج شدن پیش ازاین قفل نیست'
  else
...
local function lock_group_operator(msg, data, target)
  ...
  if group_operator_lock == 'yes' then
    return '*اپراتور پیش از این قفل است'
  else
...
local function unlock_group_operator(msg, data, target)
  ...
  if group_operator_lock == 'no' then
    return '*اپراتور پیش ازاین قفل نیست'
  else
...
local function lock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'yes' then
    return '*ریپلای پیش از این قفل است'
  else
...
local function unlock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'no' then
    return '*ریپلای پیش از این قفل نیست'
  else
...
local function lock_group_username(msg, data, target)
  ...
  if group_username_lock == 'yes' then
    return '*یوزرنیم پیش از این قفل است'
  else
...
local function unlock_group_username(msg, data, target)
  ...
  if group_username_lock == 'no' then
    return '*یوزرنیم پیش ازاین قفل نیست'
  else
...
local function lock_group_media(msg, data, target)
  ...
  if group_media_lock == 'yes' then
    return '*رسانه پیش از این قفل است'
  else
...
local function unlock_group_media(msg, data, target)
  ...
  if group_media_lock == 'no' then
    return '*رسانه پیش از این قفل نیست'
  else
...
local function lock_group_fosh(msg, data, target)
  ...
  if group_fosh_lock == 'yes' then
    return '*فحاشی پیش از این قفل است'
  else
...
local function unlock_group_fosh(msg, data, target)
  ...
  if group_fosh_lock == 'no' then
    return '*فحاشی پیش از این قفل نیست'
  else
...
local function lock_group_join(msg, data, target)
  ...
  if group_join_lock == 'yes' then
    return '*جوین قبل از این قفل است'
  else
...
local function unlock_group_join(msg, data, target)
  ...
  if group_join_lock == 'no' then
    return '*جوین قبل از این قفل نیست'
  else
...
local function lock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'yes' then
    return '*فروارد پیش از این قفل است'
  else
...
local function unlock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'no' then
    return '*فروارد پیش ازاین قفل نیست'
  else
    data[tostring(target)]['settings']['fwd'] = 'no'
    save_data(_config.moderation.data, data)
    return '*فروارد باز شد'
  end
end
...
local function lock_group_english(msg, data, target)
  ...
  if group_english_lock == 'yes' then
    return '*انگلیسی پیش  از این قفل است'
  else
...
local function unlock_group_english(msg, data, target)
  ...
  if group_english_lock == 'no' then
    return '*انگلیسی پیش از این قفل نیست'
  else
...
local function lock_group_emoji(msg, data, target)
  ...
  if group_emoji_lock == 'yes' then
    return '*ایموجی پیش از این قفل است'
  else
...
local function unlock_group_emoji(msg, data, target)
  ...
  if group_emoji_lock == 'no' then
    return '*ایموجی پیش از این قفل نیست'
  else
...
local function unlock_group_all(msg, data, target)
  ...
  local group_all_lock = data[tostring(target)]['settings']['all']
  if group_all_lock == 'no' then
    return '*همه تنظیمات پیش از این قفل نیست'
  else
    data[tostring(target)]['settings']['all'] = 'no'
    save_data(_config.moderation.data, data)
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'yes' then
    return '*حذف ربات پیش از این فعال است'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'yes'
    save_data(_config.moderation.data, data)
    return '*حذف ربات فعال شد'
  end
end
...
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return '*حذف ربات پیش از این غیرفعال است'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'no'
    save_data(_config.moderation.data, data)
    return '*حذف ربات غیرفعال شد'
  end
end
...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return 'قوانین سوپر گروه ثبت شد به'
end
...
local function set_public_membermod(msg, data, target)
  ...
  if group_public_lock == 'yes' then
    return 'گروه پیش از این عمومیه'
  else
    data[tostring(target)]['settings']['public'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'گروه حالا عمومی شد'
end
...
local function promote_admin(receiver, member_username, user_id)
  ...
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..' قبل از این مد است.')
  end
...
local function promote2(receiver, member_username, user_id)
  ...
  local member_tag_username = string.gsub(member_username, '@', '(at)')
  if not data[group] then
    return send_large_msg(receiver, 'سوپر گروه اد نشده ک.')
  end
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..' از قبل مده.')
  end
  data[group]['moderators'][tostring(user_id)] = member_tag_username
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' یکی مقامش رفت بالا.')
end
...
local function demote2(receiver, member_username, user_id)
  ...
  local group = string.gsub(receiver, 'channel#id', '')
  if not data[group] then
    return send_large_msg(receiver, 'گروه اد نشده ک.')
  end
  if not data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_tag_username..' مد نیست ک.')
  end
  data[group]['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' یکی مقامش اومد پایین.')
end
...
local function modlist(msg)
  ...
  if not data[tostring(groups)][tostring(msg.to.id)] then
    return 'گروه اد نشده ک.'
  end
  -- determine if table is empty
  if next(data[tostring(msg.to.id)]['moderators']) == nil then
    return 'هیچ مدی تو اینجا نیست.'
  end
  local i = 1
  local message = '\nلیست مدا برای ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n'
...
function get_message_callback(extra, success, result)
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
			   return send_large_msg("channel#id"..channel_id, "نمیتونی بالا مقاماتو اخراج کنی خنگ")
    end
	...
	elseif get_cmd == "setadmin" then
		...
		if result.from.username then
			text = "@"..result.from.username.." حالا یه ادمینه"
		else
			text = "[ "..user_id.." ]حالا یه ادمینه"
		end
	...
	elseif get_cmd == "demoteadmin" then
		...
		if result.from.username then
			text = "@"..result.from.username.." حالا دیگ ادمین نیست"
		else
			text = "[ "..user_id.." ] حالا دیگ ادمین نیست"
		end
	...
	elseif get_cmd == "setowner" then
		...
		local channel_id = 'channel#id'..result.to.peer_id
			...
			if result.from.username then
				text = "@"..result.from.username.." [ "..result.from.peer_id.." ] حالا صاحب گروهه"
			else
				text = "[ "..result.from.peer_id.." ] حالا صاحب گروهه"
			end
			send_large_msg(channel_id, text)
		end
	...
	elseif get_cmd == 'mute_user' then
		...
		if is_muted_user(chat_id, user_id) then
			...
			send_large_msg(receiver, "["..user_id.."] از لیست لال شده ها در اومد")
		elseif is_admin1(msg) then
			mute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] حالا لال شد")
		end
	end
end
...
local function cb_user_info(extra, success, result)
	...
	if get_cmd == "demoteadmin" then
		if is_admin2(result.peer_id) then
			return send_large_msg(receiver, "نمیتونی بالا مقاماتو اخراج کنی خنگ!")
		end
		local user_id = "user#id"..result.peer_id
		channel_demote(receiver, user_id, ok_cb, false)
		if result.username then
			text = "@"..result.username.." حالا دیه ادمین نیست"
			send_large_msg(receiver, text)
		else
			text = "[ "..result.peer_id.." ] حالا دیه ادمین نیست"
			send_large_msg(receiver, text)
		end
	elseif get_cmd == "promote" then
		if result.username then
...
local function in_channel_cb(cb_extra, success, result)
  ...
  if member then
    text = 'هیچ حاجی با @'..member..' تو اینجا نیست.'
  else
    text = 'هیچ حاجی ['..memberid..'] تو اینجا نیست.'
  end
...
elseif get_cmd == "setadmin" then
   for k,v in pairs(result) do
    ...
    if vusername == member or vpeer_id == memberid then
      ...
      channel_set_admin(channel_id, user_id, ok_cb, false)
      if v.username then
        text = "@"..v.username.." ["..v.peer_id.."] حالا ادمینه"
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] set admin @"..v.username.." ["..v.peer_id.."]")
      else
        text = "["..v.peer_id.."] حالا ادمینه"
	...
	for k,v in pairs(result) do
		...
		if vusername == member or vpeer_id == memberid then
			...
			if group_owner then
				...
				if result.username then
					text = member_username.." ["..v.peer_id.."] حالا صاحب گروهه"
				else
					text = "["..v.peer_id.."] حالا صاحب گروهه"
				end
			end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'add' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, 'خنگه از قبل اینجا اد شده.', ok_cb, false)
			end
		...
		if matches[1] == 'rem' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'خنگه اینجا اد نشده.', ok_cb, false)
			end
			print("سوپرگروه "..msg.to.print_name.."("..msg.to.id..") حذف شدحالا")
			superrem(msg)
			rem_mutes(msg.to.id)
		end
		...
		if matches[1] == "owner" then
			...
			if not group_owner then
				return "هیچکس صاحب گروه نیست.از بالا مقاما بخواین یکیو صاحب گروه کنه"
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] used /owner")
			return "صاحب این گروه ایشونه: ["..group_owner..']'
		end
		...
		if matches[1] == 'promote' then
		  if not is_momod(msg) then
			...
			if not is_owner(msg) then
				return "خنگه فقط ادمینا و صاحبا میتونن ارتقا بدن"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'promote',
		...
		if matches[1] == "setabout" and is_momod(msg) then
			...
			return "چرت و پرتات واسه توضیحات گروه ثبت شد.\n\nبرو بیرون بعد دوباره بیا چرتو پرتاتو میبنی."
		end
		...
		if matches[1] == "setusername" and is_admin1(msg) then
			local function ok_username_cb (extra, success, result)
				local receiver = extra.receiver
				if success == 1 then
					send_large_msg(receiver, "یوزر نیم ثبت شد")
				elseif success == 0 then
					send_large_msg(receiver, "نمیشه یوزر نیم ثبت کرد..دلایلشو ول کن فضول نباش")
				end
			end
			local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'setphoto' and is_momod(msg) then
			...
			return 'حالا عکس جدید گروه بفرس'
		end
		...
		if matches[1] == 'clean' then
			...
			if not is_momod(msg) then
				return "تنها صاحبا میتونن پاک کنن خنگ"
			end
			if matches[2] == 'modlist' then
				if next(data[tostring(msg.to.id)]['moderators']) == nil then
					return 'هیچ مدیر خراب شده ای تو این گروه نیست.'
				end
				for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
				...
				return 'مدیرا پاک شدن'
			end
			if matches[2] == 'rules' then
				local data_cat = 'rules'
				if data[tostring(msg.to.id)][data_cat] == nil then
					return "هیچ قانونی نزاشتی ک"
				end
				data[tostring(msg.to.id)][data_cat] = nil
				save_data(_config.moderation.data, data)
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] cleaned rules")
				return 'قانونا پاک شدن'
			end
			if matches[2] == 'about' then
				...
				local data_cat = 'description'
				if data[tostring(msg.to.id)][data_cat] == nil then
					return 'هیچ چرتو پرتی واس گروه ننوشتی ک'
				end
				data[tostring(msg.to.id)][data_cat] = nil
				save_data(_config.moderation.data, data)
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] cleaned about")
				channel_set_about(receiver, about_text, ok_cb, false)
				return "چرت و پرتا پاک شد"
			end
			if matches[2] == 'silentlist' then
				chat_id = msg.to.id
				local hash =  'mute_user:'..chat_id
					redis:del(hash)
				return "لیست لال شده ها پاک شد"
			end
			if matches[2] == 'username' and is_admin1(msg) then
				local function ok_username_cb (extra, success, result)
					local receiver = extra.receiver
					if success == 1 then
						send_large_msg(receiver, "یوزر نیم پاک شد")
					elseif success == 0 then
						send_large_msg(receiver, "نمیشه یوزر نیم پاک کرد.")
					end
				end
				local username = ""
				channel_set_username(receiver, username, ok_username_cb, {receiver=receiver})
		...
		if matches[1] == 'mute' and is_owner(msg) then
			...
			local msg_type = 'Audio'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return msg_type.." پاک کردنش روشن شد"
				else
					return "پاک کردن "..msg_type.." قبل این روشنه"
				end
			end
			...
			local msg_type = 'All'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "پاک کردن "..msg_type.." فعال شد"
				else
					return "پاک کردن "..msg_type.." قبل این فعاله"
				end
			end
		end
		if matches[1] == 'unmute' and is_momod(msg) then
			...
			local msg_type = 'Audio'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return msg_type.." پاک کردنش خاموش شد"
				else
					return "پاک کردن "..msg_type.." قبل این خاموشه"
				end
			end
			if matches[2] == 'photo' then
			local msg_type = 'Photo'
			...
			local msg_type = 'All'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return "پاک کردن "..msg_type.." غیرفعال شد"
				else
					return "پاک کردن "..msg_type.." قبل این غیرفعاله"
				end
			end
		end
		...
		if matches[1] == "silent" or matches[1] == "unsilent" and is_momod(msg) then
			...
			elseif matches[1] == "silent" or matches[1] == "unsilent" and string.match(matches[2], '^%d+$') then
				...
				if is_muted_user(chat_id, user_id) then
					...
					return "["..user_id.."] از لیست لال شده ها در اومد"
				elseif is_momod(msg) then
					mute_user(chat_id, user_id)
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] added ["..user_id.."] to the muted users list")
					return "["..user_id.."] لال شد حالا"
				end
		...
		if matches[1] == 'help' and not is_owner(msg) then
			text = "اگ میخوای راهنما رو ببینی برو توی پی ویم /superhelp بفرست بهت میگم چه غلطی بکن"
